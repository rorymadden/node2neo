...
function Node2Neo(server) {
  ...
  // just run this async -  no need to block the program
  var req = {
    url: this.server + '/db/data',
    headers: {
  ...
  request.get(req, function (err, res, body) {
    if (err) {
      console.log('Error connecting to the Neo4j database at ' + self.server);
    }
    else {
      try {
        var graph = JSON.parse(body);
        console.log('Successfully connected to the Neo4j v.' + graph.neo4j_version + ' Instance at ' + self.server);
      }
      catch (e) {
        console.log('Error parsing response from Neo4j: ' + body);
      }
